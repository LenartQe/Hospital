...
export default function DoctorPatients() {
  ...
  const load = () => {
    hospitalApi.doctor.patients().then(setPatients).catch((e) => setError(String(e.message)));
    hospitalApi.medicines.list().then(setMedicines).catch(() => {});
  };
        ...
        <Grid container spacing={3}>
            ...
            <Card sx={{ p: 2 }}>
              ...
              {patients.map((p) => (
                <MDBox
                  ...
                  sx={{ cursor: "pointer", color: String(patientId) === String(p.id) ? "#fff" : "inherit" }}
          ...
          <Grid item xs={12} md={7}>
            <Card sx={{ p: 2, mb: 2 }}>
              ...
              <MDBox mb={2}>
                <MDInput label="Titulli i diagnozës" fullWidth value={dxTitle} onChange={(e) => setDxTitle(e.target.value)} />
              </MDBox>
              <MDBox mb={2}>
                <MDInput label="Përshkrimi" fullWidth multiline rows={3} value={dxDesc} onChange={(e) => setDxDesc(e.target.value)} />
              </MDBox>
            ...
            <Card sx={{ p: 2 }}>
              ...
              <MDBox mb={2}>
                <MDInput label="Doza" fullWidth value={dosage} onChange={(e) => setDosage(e.target.value)} />
              </MDBox>
              <MDBox mb={2}>
                <MDInput label="Frekuenca" fullWidth value={frequency} onChange={(e) => setFrequency(e.target.value)} />
              </MDBox>
